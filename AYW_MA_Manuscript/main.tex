...
The first two points relate to whether propositions \ref{prop:tworegion_revenue_only} and \ref{prop:tworegion_resilience_shift} actually correspond to the hypotheses $H_0$ and $H_1$. The key point that I clarify is that this is valid because general budget expansion under the windfall cannot explain the evidence for $H_1$: although the central government does enjoy a growth in budget, this growth does not explain why the central government chooses to invest in a certain province. The synthetic control method naturally accounts for the growth in the outcome variable being a prevalent trend across all units. Likewise, revenue-enhancing reinvestment in the windfall province is certainly possible, but my theory indicates that the marginal value of revenue-enhancing reinvestment should be low compared to any number of other provinces with worse public goods. Thus a revenue-maximizing state should not be investing in Inner Mongolia to generate further revenue because the opportunity cost is higher than the revenue returns. While it is possible that the central government wanted to capture the anticipated growing demand in REEs by investing in Inner Mongolia, this should have led to a more persistent effect on rail construction, rather than the spike we see here.

The provincial-government capture of REE-linked revenue is implausible under the structure of the contemporary central-provincial fiscal relationship, which is dominated by the central state. Most of the revenue windfall should have gone to the state-owned enterprise that extracts and exports REEs of Inner Mongolia. Moreover, revenue captured by the provincial government is also first redirected to the central government, who then re-allocates the aggregated revenue to provinces. Moreover, revenue streams for the provincial government that are not directed to the central government should be controlled for by my inclusion of provincial expenditure in the SCM model.

\subsubsection{Adding Difference-in-Differences}

I also consider a difference-in-differences approach to study the cross-provincial allocation of post-windfall central spending.

To do so, I draw data on the following new variables:
\begin{itemize}
    \item \textbf{Provincial political sensitivity:} I create a pre-treatment province-level additive adjusted index combining minority population share, autonomous-region status, and border status. The logic is that these variables capture the center's ex ante assessment of political costs, rather than realized unrest after treatment. By abstracting political sensitivity away from the symptom of unrest, I avoid confounding low political sensitivity with low unrest, as highly sensitive regions may face lower unrest because of existing investment in resilience. This measure is also consistent with \textcite{freeman2012blood}'s work, which argues that China's central government has used special fiscal allocations to ethnic regions to promote national integration and inter-ethnic harmony. The use of autonomous-region status and minority share also follows the Chinese state's own logic used to identify regions requiring differentiated governance and fiscal treatment. 

    \item \textbf{Marginal fiscal return:}
    \item \textbf{Central-to-provincial transfers:} I use central-to-provincial transfer data from 1995 to 2020, drawn from the Ministry of Finance annual yearbook.
    \item \textbf{Provincial exposure to the REE windfall prior to treatment:} I measure this by the approximate proportion of national rare-earth production from each province in the year 2010, drawn from the US Geological Survey. This is of course time-invariant, so as to avoid endogenizing the effects of the windfall on regional quotas.
    \item \textbf{Provincial fiscal demand:} I control for this determinant of central-to-provincial transfer size by measuring the residualized fiscal gap ratio, which I implement via a first-stage model regressing the fiscal gap ratio (fiscal gap as a proportion of budget) against GDP, urban concentration, and industrial output. The logic is that provinces may experience higher or lower central allocations not because they are suffering budget shortfalls, not because the center wants to invest in either revenue or resilience.
    \item \textbf{Binary Autonomous-Region Indicator:} I implement this as a robustness check for political sensitivity.
\end{itemize}

Because consistent province-year rare-earth production and quota data are unavailable, the exposure measure is not intended to recover exact province-level rare-earth revenue. Rather, following the logic of continuous-treatment difference-in-differences and shift-share designs, it assigns provinces different pre-treatment intensities of exposure to a common post-2010 shock. In such designs, the key identifying variation comes from plausibly exogenous differences in exposure intensity across units, not from exact accounting of the treatment flow itself; exposure-share designs similarly use predetermined local shares to weight common shocks \parencite{callawayDifferenceinDifferencesContinuousTreatment}. The measure therefore needs ordinal precision more than exact cardinal precision: it must distinguish major rare-earth provinces from moderate, minor, and non-producing provinces, but the estimates should not be interpreted as precise tonnage or revenue measures. To avoid relying on false precision, I use both continuous estimated exposure shares and coarser categorical specifications, so that the results depend on broad cross-provincial exposure rankings rather than small differences between noisy estimates.
