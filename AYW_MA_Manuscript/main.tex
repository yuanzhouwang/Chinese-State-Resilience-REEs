...
\begin{itemize}
    \item \textbf{Provincial political sensitivity:} I measure this via
    \item \textbf{Marginal fiscal return:}
    \item \textbf{Central-to-provincial transfers:} Because direct province-level central transfer data are unavailable before 2016, I use a residualized fiscal gap ratio. To do so, I implement a first-stage model regressing the fiscal gap ratio against its main predictors, excluding the variables of the second-stage test.
    \item \textbf{Provincial exposure to the REE windfall:}
\end{itemize}
